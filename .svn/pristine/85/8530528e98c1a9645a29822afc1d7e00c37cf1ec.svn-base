...
import it.eng.care.domain.flow.core.controller.FormFlowRegionController;
...
@RestController
@RequestMapping("/fm/FormFlowRegionDTO")
public class FormFlowRegionControllerImpl implements FormFlowRegionController {

    private static final Logger LOGGER = LoggerFactory.getLogger(FormFlowRegionControllerImpl.class);
    ...
    @Autowired
    private JsonSchemaUtils jsonSchemaUtils;

    public FormFlowRegionControllerImpl() {
    ...
    public void post() {
        ...
        conversionService.registerConverter(FlowTableFieldDO.class, FlowTableFieldDTO.class, flowTableFieldDOtoFlowTableFieldDTO);
        conversionService.registerConverter(FlowTableFieldDTO.class, FlowTableFieldDO.class, flowTableFieldDTOtoFlowTableFieldDO);
    }
    ...
    public SaveOperationResult<FormFlowDTO> save(@RequestBody FormFlowDTO inputDTO) {
        String newEntity = "";
        try {
	        if (inputDTO.getId() == null || inputDTO.getId().trim().equals("")) {
	            newEntity = formFlowService.saveEntity(inputDTO,true);
	        } else {
	            newEntity = formFlowService.updateEntity(inputDTO);
	        }
        } catch (Exception e) {
        ...
        baseSearchInput.setValue("flowNameLike", newEntity);
        ...
        try {
            jsonSchemaUtils.generateSchema(formFlowPair.getFirst().get(0));
    ...
    public SearchOperationResult<FormFlowDTO> search(@RequestBody BaseSearchInput searchInput) {
        ...
        searchInput.setValue("type", "REGIONE");
    ...
    @ResponseBody
    public void createTable(@RequestBody FormFlowDTO inputDTO) {
        //TODO se la tabella esiste gia bisogna lancare un ALTERTABLE
       // formFlowService.createTable(inputDTO);

    }

    /*@Override
    @PostMapping("/importFormFlowJson")
    @ResponseBody
    public SaveOperationResult<FormFlowDTO> importFormFlowJson(Json formFlowJson) {

        Gson gson = new Gson();
        FormFlowDTO formFlowDTO = gson.fromJson(String.valueOf(formFlowJson), FormFlowDTO.class);

        if (formFlowDTO.getId() == null || formFlowDTO.getId().trim().equals("")) {
            formFlowService.createEntity(formFlowDTO);
        } else {
            formFlowService.updateEntity(formFlowDTO);
        }

        return SaveOperationResult.success(formFlowDTO);

    }

    @Override
    @PostMapping("/exportFormFlow")
    @ResponseBody
    public Json exportFormFlowJson(@RequestBody FormFlowDTO inputDTO) {
        Gson gson = new Gson();
        String formFlowDTOjson = gson.toJson(inputDTO);
