...
public class ProfilaturaExportConverter implements PrivacyManagerLogConverter<byte[]>{

	@Override
	public List<EventResponseAM> convertResponse(byte[] output, Object[] input) {
		
		TabgenValueFilter estrazione = (TabgenValueFilter) input[0];
		...
		attac.setName("Export Profilatura");
		...
		item.setKey("Profilatura");
		item.setValue(String.valueOf(estrazione.getTabgenId()));
