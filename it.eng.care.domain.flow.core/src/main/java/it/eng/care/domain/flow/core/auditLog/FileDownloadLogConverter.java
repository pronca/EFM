...
public class FileDownloadLogConverter implements PrivacyManagerLogConverter<byte[]>{
	...
	public List<EventResponseAM> convertResponse(byte[] output, Object[] input) {
		...
		if (output != null) {
			...
			attac.setName("Download Log File Caricamento Flusso " + flowFileUploadRequest.getFlow().getName());
	...
	public EventResourceAM convertRequest(Object[] args) {
		...
		request.setRequest("Download Log file caricamento flusso");
